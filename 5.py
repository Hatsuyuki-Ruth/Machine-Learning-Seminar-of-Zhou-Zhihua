# Author: Ruth (hatsuyukiw@gmail.com)
...
def summarize(dataset, num_class):
    dataset_transpose = list(zip(*dataset))
    discrete_data = dataset_transpose[:2500]
    continuous_data = dataset_transpose[2500:]
    summaries = [[(sum(attribute) + 1) / float(len(attribute) + num_class)] for attribute in discrete_data]
...
def summarizeByClass(dataset, y):
    ...
    for classValue, instances in iter(separated.items()):
        summaries[classValue] = summarize(instances, len(separated))
...
def calculateProbability(x, type, p_1=0, mean=0, stdev=1):
    if type == 'cont':
        ...
        return robust_log((1 / (math.sqrt(2 * math.pi) * stdev))) + exponent
    else:
        if x == 0:
            return 1 - p_1
        else:
    ...
    for classValue, classSummaries in iter(summaries.items()):
        ...
        for i in range(len(classSummaries)):
            if i < 2500:
                ...
                x = inputVector[i]
                probabilities[classValue] += calculateProbability(x, 'cont', mean=mean, stdev=stdev)
